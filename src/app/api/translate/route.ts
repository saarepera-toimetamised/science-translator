...
async function scrapeArticle(url: string, estonianTitle?: string) {
  // Sinu kraapimise funktsioon jääb samaks.
  // Ma ei muuda siin midagi, sest see on juba päris hästi ehitatud.
  // Probleem ei ole kraapijas endas, vaid selles, kuidas me selle ebaõnnestumist käsitleme.
  try {  
    ...
    if (action === 'scrape') {
      ...
      for (let i = 0; i < urls.length; i++) {
        ...
        try {
          ...
          // --- MINU MUUDATUS ALGAB SIIT ---
          console.error(`Scraping failed for ${url}, creating fallback task for AI:`, error);
          
          // Instead of pushing an error, we create a special article object
          // that instructs Gemini to find the content itself.
          articles.push({
            title: `(Kraapimine ebaõnnestus) ${estonianTitle || url}`,
            content: `KRIITILINE JUHIS: Selle artikli kraapimine aadressilt ${url} EBAÕNNESTUS tõenäoliselt turvablokeeringu (nt Cloudflare) tõttu. ÄRA VASTA, ET SA EI SAANUD SISU KÄTTE, VÕI EI SAA URL-E AVADA. Sinu ülesanne on käituda nüüd teisiti: kasuta internetiotsingut, et leida selle artikli sisu alternatiivsetest allikatest. Otsi pealkirja või URL-is sisalduvate märksõnade abil. Kui oled identse artikli sisu leidnud, tõlgi see vastavalt kõikidele teistele antud reeglitele.`,
            url: url,
            estonianTitle: estonianTitle,
          });
          // We also log the original error for debugging, but don't show it to the user
          errors.push({ url, error: error instanceof Error ? error.message : 'Unknown error' });
          // --- MINU MUUDATUS LÕPPEB SIIT ---
        }
      }

      // We no longer consider a scrape failure as a total failure.
      // The process continues as long as we have article tasks (even fallback ones).
      if (articles.length === 0) {
        return NextResponse.json(
          { error: 'Could not create any article processing tasks.', details: errors },
          { status: 500 }
        );
      }
      ...
      return NextResponse.json({
        ...
        // We can still optionally return errors for the UI to show a warning
